...
function main(config) {
    ...
    const 节点黑名单 = "(阻止|China|🇨🇳|节点|过滤|剩余|流量|距离|下次|重置|重新|订阅|导入|套餐|到期|跳转|域名|请勿|邀请|好友|关注|频道|收费|就说明|被骗|续费|更新|地址|官网|下载|群组|永久|长期|中继|更换|协议|软件|教程|Lite|ali|null)";
    ...
    const 欧盟正则 = "^(?!(?:.*(?:马来|印度|美|流量))).*(奥|比|保|克罗地亚|塞|捷|丹|爱沙|芬|法|德|希|匈|爱尔|意|拉|立|卢|马耳他|马其顿|荷|波|葡|罗|斯洛伐|斯洛文|西|瑞|英|🇧🇪|🇨🇿|🇩🇰|🇫🇮|🇫🇷|🇩🇪|🇮🇪|🇮🇹|🇱🇹|🇱🇺|🇳🇱|🇵🇱|🇸🇪|🇬🇧|CDG|FRA|AMS|MAD|BCN|FCO|MUC|BRU|GB|FR|DE|NL|RU|LV|SE|LT|AU|NZ)";
    const 优选机场正则 = "(💧|🐔|❤️|💴)".replace(/️/g, "️?");
    const 优选网络正则 = "(CT|电信|BGP|三网|推荐|AWS|高速|专线)";
    const 排除正则 = "(家|住|直|x|倍|免费)";
    ...
    config["proxy-groups"] = [
        ...
        // ▸ 自选策略组 ----------
        { name: "📹 视频平台", type: "select", proxies: 策略组, "default-selected": "🚀 节点选择", icon: 图标库 + "Netflix_Letter.png" },
        { name: "📲 电报飞机", type: "select", proxies: 策略组, "default-selected": "🚀 节点选择", icon: 图标库 + "Telegram_X.png" },
        { name: "📲 社交媒体", type: "select", proxies: 策略组, "default-selected": "🇯🇵 日本故转", icon: 图标库 + "TikTok.png" },
        { name: "🎵 音乐服务", type: "select", proxies: 策略组, "default-selected": "🇯🇵 日本故转", icon: 图标库 + "Music_Enhance.png" },
        ...
        // ▸ 其他策略组 ----------
        { name: "🖥️ DNS解析", type: "select", proxies: ["🖥️ 服务节点", "🚀 节点选择"], icon: 图标库 + "Cloudflare.png" },
        ...
        ...创建地区分组("🇯🇵 日本", "Japan.png", 日本_List, `${日本正则}`, `(?=.*${优选网络正则})(?=.*${优选机场正则})`, `(?!.*${排除正则})`),
        ...创建地区分组("🇹🇼 台湾", "Taiwan.png", 台湾_List, `${台湾正则}`, `${优选机场正则}`, `(?!.*${排除正则})`),
        ...创建地区分组("🇰🇷 韩国", "Korea.png", 韩国_List, `${韩国正则}`, `${优选机场正则}`, `(?!.*${排除正则})`),
        ...创建地区分组("🇪🇺 欧盟", "European_Union.png", 欧盟_List, `${欧盟正则}`, `${优选机场正则}`, `(?!.*${排除正则})`),
    ];
