...
function main(config) {
    ...
    config["proxy-groups"] = [
        ...
        // ▸ 生成地区组 ----------
        ...创建地区分组("🇭🇰 香港", "Hong_Kong.png", 香港_List, 香港_List, `${香港正则}`),
        ...创建地区分组("🇸🇬 狮城", "Singapore.png", 狮城_List, 狮城_List, `${狮城正则}`),
        ...创建地区分组("🇺🇸 美国", "United_States.png", 美国_List, 美国_List, `${美国正则}`),
        ...创建地区分组("🇯🇵 日本", "Japan.png", 日本_List, 日本_List, `${日本正则}`),
        ...创建地区分组("🇹🇼 台湾", "Taiwan.png", 台湾_List, 台湾_List, `${台湾正则}`),
        ...创建地区分组("🇰🇷 韩国", "Korea.png", 韩国_List, 韩国_List, `${韩国正则}`),
        ...创建地区分组("🇪🇺 欧盟", "European_Union.png", 欧盟_List, 欧盟_List, `${欧盟正则}`),
        // ▸ 其他策略组 ----------
    ...
    function 创建地区分组(地区名, 地区图标, 选择池, 测速池, 地区正则) {
        const 测速池重排 = 测速池.includes("🈚️ 假节点") && 测速池.length > 1
            ? [...测速池.filter(p => p !== "🈚️ 假节点"), "🈚️ 假节点"]
            : 测速池;
        return [
            { name: `${地区名}节点`, type: "select", use: 外部订阅, filter: 地区正则, proxies: [`${地区名}自动`, `${地区名}散列`, `${地区名}轮询`, ...选择池], icon: 图标库 + 地区图标 },
            { name: `${地区名}自动`, type: "url-test", use: 外部订阅, filter: 地区正则, proxies: 测速池重排, hidden: true, icon: 图标库 + 地区图标, interval: 300, url: "https://cp.cloudflare.com/generate_204", timeout: 2000, "max-failed-times": 3, method: "HEAD", lazy: false },
            { name: `${地区名}散列`, type: "load-balance", strategy: "consistent-hashing", use: 外部订阅, filter: 地区正则, proxies: 测速池重排, hidden: true, icon: 图标库 + 地区图标, interval: 300, url: "https://cp.cloudflare.com/generate_204", timeout: 2000, "max-failed-times": 3, method: "HEAD", lazy: true },
            { name: `${地区名}轮询`, type: "load-balance", strategy: "round-robin", use: 外部订阅, filter: 地区正则, proxies: 测速池重排, hidden: true, icon: 图标库 + 地区图标, interval: 300, url: "https://cp.cloudflare.com/generate_204", timeout: 2000, "max-failed-times": 3, method: "HEAD", lazy: true }
        ];
    }
