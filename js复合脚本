//https://raw.githubusercontent.com/TYDMX/config-files/refs/heads/main/1/js单订阅脚本
function main(config) {
    ...
    // --- 【节点筛选正则表达式】 ---
    const 香港正则 = '(?i)(港|🇭🇰|HK|Hong|HKG)';
    const 狮城正则 = '(?i)(坡|🇸🇬|SG|Sing|SIN|XSP)';
    const 美国正则 = '(?i)(美|🇺🇸|US|USA|JFK|LAX|ORD|ATL|DFW|SFO|MIA|SEA|IAD)';
    const 日本正则 = '(?i)(日|🇯🇵|JP|Japan|NRT|HND|KIX|CTS|FUK)';
    const 韩国正则 = '(?i)(韩|🇰🇷|韓|首尔|南朝鲜|KR|KOR|Korea)';
    const 台湾正则 = '(?i)(台|🇹🇼|TW|tai|TPE|TSA|KHH)';
    const 欧盟正则 = "(奥|比|保|克罗地亚|塞|捷|丹|爱沙|芬|法|德|希|匈|爱尔|意|拉|立|卢|马其它|荷|波|葡|罗|斯洛伐|斯洛文|西|瑞|英|🇧🇪|🇨🇿|🇩🇰|🇫🇮|🇫🇷|🇩🇪|🇮🇪|🇮🇹|🇱🇹|🇱🇺|🇳🇱|🇵🇱|🇸🇪|🇬🇧|CDG|FRA|AMS|MAD|BCN|FCO|MUC|BRU|GB|FR|DE|NL|RU|LV|SE|LT|AU|NZ)";
    const 欧盟排除 = "(马来|印度)";
    
    const 香港筛选 = 原始节点.filter(n => new RegExp("(港|🇭🇰|HK|Hong|HKG)", "i").test(n));
    const 狮城筛选 = 原始节点.filter(n => new RegExp("(坡|🇸🇬|SG|Sing|SIN|XSP)", "i").test(n));
    const 美国筛选 = 原始节点.filter(n => new RegExp("(美|🇺🇸|US|USA|JFK|LAX|ORD|ATL|DFW|SFO|MIA|SEA|IAD)", "i").test(n));
    const 日本筛选 = 原始节点.filter(n => new RegExp("(日|🇯🇵|JP|Japan|NRT|HND|KIX|CTS|FUK)", "i").test(n));
    const 台湾筛选 = 原始节点.filter(n => new RegExp("(台|🇹🇼|TW|tai|TPE|TSA|KHH)", "i").test(n));
    const 韩国筛选 = 原始节点.filter(n => new RegExp("(韩|🇰🇷|韓|首尔|南朝鲜|KR|KOR|Korea)", "i").test(n));


    const 欧盟筛选 = 原始节点.filter(n => new RegExp(欧盟正则, "i").test(n) && !new RegExp(欧盟排除, "i").test(n));
    ...
    // --- 【策略组代理列表模板】 ---
    const 节点选择池 = ["🇭🇰 香港节点", "🇺🇸 美国节点", "🇸🇬 狮城节点", "🇯🇵 日本节点", "🇹🇼 台湾节点", "🇰🇷 韩国节点", "🇪🇺 欧盟节点", "🌐 全部节点"];
    ...
    config["proxy-groups"] = [
        ...
        // --- 【其他策略组】 ---
        { name: "🌐 全部节点", type: "select", proxies: ["🈚️ 假节点", ...原始节点], icon: 图标库 + "Clubhouse.png" },
    ...
    function 创建地区分组(地区名, 地区图标, 地区节点池, 外部地区节点池) {
        return [
            { name: `${地区名}节点`, type: "select", use: 外部订阅, filter: 外部地区节点池, proxies: [`${地区名}自动`, `${地区名}散列`, `${地区名}轮询`, "🈚️ 假节点", ...地区节点池], icon: 图标库 + 地区图标 },
